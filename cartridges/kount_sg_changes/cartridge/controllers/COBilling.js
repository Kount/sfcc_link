...
function handlePaymentSelection(cart) {
    ...
    var RISresult = Kount.preRiskCall(cart);
    if(RISresult && RISresult.KountOrderStatus == 'DECLINED') {
        return result = {
            error: true,
            KountOrderStatus: RISresult.KountOrderStatus
        };
    }
    ...
    app.getForm('billing').handleAction({
        ...
        save: function () {
            Transaction.wrap(function () {
                ...
                (result = handlePaymentSelection(cart)).error) {// Performs payment method specific checks, such as credit card verification.
                    returnToForm(cart, result);
                } else {
